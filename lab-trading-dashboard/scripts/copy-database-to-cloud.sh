#!/usr/bin/env bash
# Copy olab from DB server (150.241.245.36) to app server (150.241.244.130).
...
if [ -f /etc/lab-trading-dashboard.env ] && [ "$(whoami)" = "root" ] && [[ "$REPO_ROOT" == /opt/* ]]; then
  ...
  if PGPASSWORD="$DB_PASS" pg_dump -h 150.241.245.36 -U postgres -d olab -F c -f /tmp/olab.dump 2>/dev/null; then
    echo "→ Dump created. Restoring locally..."
    sudo -u postgres psql -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = 'olab' AND pid <> pg_backend_pid();" 2>/dev/null || true
    sudo -u postgres psql -c "DROP DATABASE IF EXISTS olab;"
    sudo -u postgres psql -c "CREATE DATABASE olab OWNER postgres;"
    sudo -u postgres pg_restore -d olab -F c /tmp/olab.dump || true
...
if SSHPASS="$DB_SERVER_PASSWORD" sshpass -e ssh -o StrictHostKeyChecking=accept-new "$DB_SERVER" \
  "PGPASSWORD='IndiaNepal1-' pg_dump -h 127.0.0.1 -U postgres -d olab -F c -f /tmp/olab.dump && ls -la /tmp/olab.dump"; then
  echo "→ Step 2: Copy dump to app server..."
  SSHPASS="$DB_SERVER_PASSWORD" sshpass -e scp -o StrictHostKeyChecking=no "$DB_SERVER:/tmp/olab.dump" /tmp/olab.dump
  SSHPASS="$DEPLOY_PASSWORD" sshpass -e scp -o StrictHostKeyChecking=no /tmp/olab.dump "$APP_SERVER:/tmp/olab.dump"
else
