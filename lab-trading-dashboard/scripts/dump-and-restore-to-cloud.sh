...
SSHPASS="$DEPLOY_PASSWORD" sshpass -e scp -o StrictHostKeyChecking=no "$DUMP_FILE" "$DEPLOY_HOST:/tmp/olab.dump"

echo "→ Restoring on cloud and restarting app..."
SSHPASS="$DEPLOY_PASSWORD" sshpass -e ssh -o StrictHostKeyChecking=no "$DEPLOY_HOST" "sudo -u postgres psql -c \"SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = 'olab' AND pid <> pg_backend_pid();\" 2>/dev/null || true; sudo -u postgres psql -c 'DROP DATABASE IF EXISTS olab;'; sudo -u postgres psql -c 'CREATE DATABASE olab OWNER postgres;'; sudo -u postgres pg_restore -d olab -F c /tmp/olab.dump || true; sudo systemctl restart lab-trading-dashboard"
rm -f "$DUMP_FILE"
