...
# the dump file at /tmp/olab.dump (e.g. from scp root@150.241.245.36:/tmp/olab.dump /tmp/ then scp to app server).
# Usage: sudo -u postgres bash -c './restore-db-on-server.sh'  OR  run as root and it uses sudo -u postgres.

set -euo pipefail
DUMP="${1:-/tmp/olab.dump}"
if [ ! -f "$DUMP" ]; then
  echo "Usage: $0 /path/to/olab.dump"
  echo "Dump file not found: $DUMP"
  echo "First copy the dump from your DB server, e.g.:"
  echo "  scp root@150.241.245.36:/tmp/olab.dump /tmp/"
  exit 1
fi
echo "→ Terminating connections to olab..."
