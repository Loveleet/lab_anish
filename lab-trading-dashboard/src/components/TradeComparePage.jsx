...
const TradeComparePage = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [tradesRes, machinesRes] = await Promise.all([
        fetch(`${getApiBaseUrl()}/api/trades`),
        fetch(`${getApiBaseUrl()}/api/machines`)
      ]);
  ...
  const fetchRowDetail = useCallback(
    async (row) => {
      ...
      try {
        const res = await fetch(`${getApiBaseUrl()}/api/bot-event-logs?uid=${encodeURIComponent(uid)}&limit=500`);
