...
const getPairStats = (pair, trades) => {
  ...
  const hedgeCount = pairTrades.filter(t => t.Hedge === true).length;
  ...
  const totalProfitAmount = pairTrades.filter(t => t.Pl_after_comm > 0).reduce((sum, t) => sum + (t.Pl_after_comm || 0), 0);
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center gap-2 justify-between w-full">
        <input
          type="text"
          className="px-3 py-2 rounded border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-black dark:text-white w-full max-w-xs focus:outline-none focus:ring-2 focus:ring-blue-400"
          placeholder="Search pair..."
          value={search}
          onChange={e => setSearch(e.target.value)}
        />
        <select value={sortOption} onChange={e => handleSortChange(e.target.value)} className="px-2 py-1 rounded border bg-white text-black dark:bg-gray-900 dark:text-white">
...
const PairStatsGrid = ({ onPairSelect, candleType, interval, trades = [], selectedPair = null, previewMode = false, filterBar, darkMode, groupModeEnabled, selectedGroupPairs = [], setSelectedGroupPairs, ...rest }) => {
  ...
  function getHedgePL(pair, trades) {
    const hedgeTrades = previewMode
      ? trades.filter(t => t.Pair === pair && t.Hedge === true && typeof t.Pl_after_comm === 'number')
      : filterTrades(trades.filter(t => t.Pair === pair && t.Hedge === true && typeof t.Pl_after_comm === 'number'));
    return hedgeTrades.reduce((sum, t) => sum + t.Pl_after_comm, 0);
  }
  ...
  // After sorting and filtering pairs
  if (sortedPairs.length === 0) {
    return (
      <div className={`p-2 sm:p-4 min-h-screen transition-colors duration-300 ${darkMode ? 'bg-black text-white' : 'bg-white text-black'}`}
           style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, fontWeight: 600 }}>
        No trades for this stat.
      </div>
    );
  }
